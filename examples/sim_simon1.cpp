...
void defineDefaultParameters(boost::property_tree::ptree const& pt)
{
	...
	string file_prefix = "simon1";
	NeuronID N_pre = 50;   // number of inputs
	NeuronID seed = 1;	  // random initialisation seed
}


double getSamplinginterval(boost::property_tree::ptree const& pt, string path)
{
...
//        }
	...
	try
	{
		//sprintf(strbuf, "simon1.%d.log", world.rank());
...
//	cout << "IFNeuron.mem: " << detector_neuron->get_mem(0) << endl;
...
//	}
	...
	if (true)
	{
		...
		while (ssStart >> buf1)
		{
			...
			++counter;
			cout << "Reading in starttime: " << buf1 << " ...and Stoptime: " << buf2 << endl;
		}
		...
		PopulationRateMonitor * rmon_in = new PopulationRateMonitor( poisson, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.inputs.samplinginterval_poprate") );
		...
		RateMonitor * rmon_out = new RateMonitor( detector_neuron, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.outputs.samplinginterval_rate") );
		...
		tmpstr = simparams.get<string>("general.outfileprefix");
		tmpstr += ".ampa";
		AmpaMonitor * amon = new AmpaMonitor( detector_neuron, 0, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.outputs.samplinginterval_ampa")/dt ); // divide by dt because constructor interfaces are not (yet) normed across classes.
		amon->set_recording_times(starttimes,stoptimes);

		tmpstr = simparams.get<string>("general.outfileprefix");
		tmpstr += ".nmda";
		NmdaMonitor * nmon = new NmdaMonitor( detector_neuron, 0, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.outputs.samplinginterval_nmda")/dt ); // divide by dt because constructor interfaces are not (yet) normed across classes.
		nmon->set_recording_times(starttimes,stoptimes);


		//tmpstr = simparams.get<string>("general.outfileprefix");
		//tmpstr += ".izhi";
		//IzhiMonitor * itemon = new IzhiMonitor( detector_neuron, 0, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.outputs.samplinginterval_ampa")/dt ); // divide by dt because constructor interfaces are not (yet) normed across classes.



		tmpstr = simparams.get<string>("general.outfileprefix");
		tmpstr += ".weightsum";
		WeightSumMonitor * weightsum = new WeightSumMonitor( con1, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.con1.samplinginterval_weightsum") );

		tmpstr = simparams.get<string>("general.outfileprefix");
		tmpstr += ".weightstats";
		WeightStatsMonitor * weightstats = new WeightStatsMonitor( con1, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.con1.samplinginterval_weightstats") );

		tmpstr = simparams.get<string>("general.outfileprefix");
		tmpstr += ".weightmatrix";
		WeightMatrixMonitor * weightmatrix = new WeightMatrixMonitor( con1, tmpstr.c_str(), getSamplinginterval(simparams,"recordings.con1.samplinginterval_weightmatrix") );
	}
	...
	sys->run(simparams.get<double>("general.simtime"));
